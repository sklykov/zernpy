...
def test_random_fitting():
    ...
    for i in range(4, 8, 1):
        if i == 4:
            height = 147; width = 147; crop_r = 1.0; strict_border = False; mdp = 10.0; stop_warns = False
        elif i == 5:
            height = 160; width = 160; crop_r = 0.98; strict_border = False; mdp = 20.0; stop_warns = True
        elif i == 6:
            height = 201; width = 204; crop_r = 0.95; strict_border = True; mdp = 15.0; stop_warns = True
        elif i == 7:
            height = 222; width = 224; strict_border = False; crop_r = 0.8; mdp = 30.0; stop_warns = True
    ...
    random_phases_image, random_amplitudes, polynomials_tuple = generate_random_phases(img_height=181, img_width=181)
    fitted_amplitudes, _ = fit_polynomials(random_phases_image, polynomials_tuple)
    max_fit_ampl = np.max(fitted_amplitudes); max_scr_ampl = np.max(random_amplitudes)
    min_fit_ampl = np.min(fitted_amplitudes); min_scr_ampl = np.min(random_amplitudes)
    if abs(max_src_ampl) < abs(min_scr_ampl):
        scr_ampl = min_scr_ampl; fit_ampl = min_fit_ampl
    else:
        scr_ampl = max_scr_ampl; fit_ampl = max_fit_ampl
    same_sign = False
    if scr_ampl < 0.0 and fit_ampl < 0.0:
        same_sign = True
    elif scr_ampl > 0.0 and fit_ampl > 0.0:
        same_sign = True
    assert same_sign, ("\n Fitted and source amplitudes have abs. maximum values with different"
                       + f" signs: source ampl.: {scr_ampl}, fitted ampl.: {fit_ampl}")
