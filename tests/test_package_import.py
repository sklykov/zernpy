...
def test_initialization():
    try:
        ...
        zpsf.set_physical_props(NA=0.85, wavelength=0.4, expansion_coeff=0.01, pixel_physical_size=0.4/4.9)
        zpsf.set_calculation_props(kernel_size=17, n_integration_points_r=100, n_integration_points_phi=180)
        zpsf.calculate_psf_kernel(normalized=True, verbose_info=True); zpsf.crop_kernel()
    except ImportError:
        import os
